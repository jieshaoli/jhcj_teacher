...
@interface livingRoomVController ()<
...
@implementation livingRoomVController
...
- (void)initLivingConfig{
    ...
    //initLivingObject
    self.mediaSession = [[PLMediaStreamingSession alloc] initWithVideoCaptureConfiguration:videoCaptureConfiguration audioCaptureConfiguration:audioCaptureConfiguration videoStreamingConfiguration:videoStreamingConfiguration audioStreamingConfiguration:audioStreamingConfiguration stream:nil];
    
//    self.streamSession = [[PLStreamingSession alloc] initWithVideoStreamingConfiguration:_videoStreamConfiguration audioStreamingConfiguration:_audioStreamingConfiguration stream:nil];
}

//- (void)initVideoView{
//    [self.view addSubview:self.mediaSession.previewView];
//}

- (void)setCourseInfo:(NSDictionary *)courseInfo{
    _courseInfo = courseInfo;
    _pushUrl = courseInfo[@"detail"][@"push_url"];
    _pushURL = [NSURL URLWithString:self.pushUrl];
}
...
//    button.center = CGPointMake(CGRectGetMidX([UIScreen mainScreen].bounds), CGRectGetHeight([UIScreen mainScreen].bounds) - 88);
...
//    [self.view insertSubview:_mediaSession.previewView atIndex:0];
    [self.view addSubview:_mediaSession.previewView];
