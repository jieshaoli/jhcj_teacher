...
export default {
  ...
  activated() {
    this.ifReload = true;
    if (this.courseList.length <= 0 || this.ifReload) {
      ...
      if (type === 1) {
        ...
        if (idStr && idStr !== undefined && idStr !== null) {
          if (status === 3) {
            ...
            MessageBox.confirm('确定要在移动端直播吗？')
              .then((res) => {
                console.log(res);
                this.js2native(JSON.stringify({type: 'toLiving', info: course_info}));
              })
              .catch((rej) => {
                console.log(rej);
              });
          } else if (status == 2) {
            MessageBox.confirm('有其他人正在直播，这样会影响正在直播的人，确定现在要在移动端直播吗？')
              .then((res) => {
                console.log(res);
                this.js2native(JSON.stringify({type: 'toLiving', info: course_info}));
              })
        ...
        // Toast('暂不支持视频课');
        noti.$emit('route', { path: '/videoPlay', info: course_info });
      }
    },
    loadTop() {
      console.log('加载更多数据');
